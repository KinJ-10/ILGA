...
static const struct bt_data ad[] = {
	BT_DATA(BT_DATA_NAME_COMPLETE, "IL-CS-TAG", sizeof(sample_str) - 1),
};
...
#if defined(CONFIG_IL_CS_RAW_DIAGNOSTICS)
	latest_peer_meta = (struct il_cs_peer_meta){
		.magic = IL_CS_PEER_META_MAGIC,
		.version = IL_CS_RAW_VERSION,
		.flags = IL_CS_PEER_FLAG_NO_DATA,
...
#if defined(CONFIG_IL_CS_RAW_DIAGNOSTICS)
			latest_peer_meta.flags = IL_CS_PEER_FLAG_OVERFLOW;
#endif
		}
	}
...
int main(void)
{
	...
	printk("Starting ILGA Channel Sounding Phase 0 (TAG/Reflector)\n");
	...
	while (true) {
		...
		write_params.length = sizeof(latest_peer_meta);
		write_params.data = &latest_peer_meta;
		write_params.offset = 0;

		err = bt_gatt_write(connection, &write_params);
		if (err) {
			printk("Metadata write failed (err %d)\n", err);
			return 0;
		}

		err = k_sem_take(&sem_written, K_SECONDS(10));
		if (err) {
			printk("Timed out during metadata write\n");
			return 0;
		}
#endif
