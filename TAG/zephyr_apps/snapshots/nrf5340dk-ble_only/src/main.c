...
/* ============================================================
 * ここから下は BLE サンプル側 (元: マージ用_BLE.txt) をほぼそのまま
 * ==========================================================*/
...
static const struct bt_uuid_128 vnd_signed_uuid = BT_UUID_INIT_128(
	BT_UUID_128_ENCODE(0x13345678, 0x1234, 0x5678, 0x1334,
					   0x56789abcdef3));
...
int main(void)
{
	...
	while (1)
	{
		 ...
		 * For testing purposes, we send a manual update
		 * notification every second.
		 * In production `bt_cts_send_notification` should only be
		 * used when time is changed
		 */
