...
- 2026-09-25: 診断版のNCS build、LOCATORへのflash、TAGとのBLE / CS接続、1 m UART実測まで完了。20完全フレームで抽出スロット欠落0、ファームIFFTとオフライン再計算は1 mm未満で一致した。1 m中央値は公式IFFT 3.220 m、早期IFFT候補4.466 m、公式位相4.699 m、ロバスト位相候補4.514 m、RTT 9.941 mで、追加候補は改善しなかった。1 m一点のoffset補正と現候補の実機採用は見送り、保存IQで候補探索を継続した。
- 同ログの全IFFTプロファイル解析では、真値1 m周辺に局所ピークは0/20件、主ローブは約4.4 m、フレーム間形状相関0.964だった。支配IFFTとロバスト位相は相関0.905、中央値差-0.034 mで同じ約4.5 mの実効遅延を示す。閾値20通りの掃引でも改善しなかった。
- 同日、同じ向き・高さ・見通しで0.5 / 1.0 / 1.5 mを各1回取得した。公式IFFT中央値は1.464 / 3.220 / 5.114 m、公式位相傾きは3.301 / 4.699 / 6.525 mで距離順序が成立した。0.5 mと1.5 mだけで一次補正し、未使用の1.0 mを予測した結果は、公式IFFT 0.981 m（誤差-0.019 m）、公式位相0.934 m（誤差-0.066 m）だった。各距離1設置の暫定結果であり、絶対距離精度の成立判定と校正式採用は再設置再現性の確認まで保留する。
- 同じ3距離の再設置試験を各21完全フレームで実施した。公式IFFTは1.814 / 2.165 / 2.928 m、公式位相傾きは3.325 / 4.665 / 5.947 mで距離順序は成立した。再試験の端点補正による1.0 m予測は公式IFFT 0.815 m（誤差-0.185 m）、公式位相1.011 m（誤差+0.011 m）だった。公式IFFTは初回との差が最大2.186 mで再設置依存が大きく、第一候補を公式位相傾きの時間窓中央値へ変更する。固定の共通校正式は未成立で、配置変化の品質判定が必要である。
