...
### 6.6 2026-09-16 見通し0.5～3.0 m静止評価

TAGをモバイルバッテリー給電、LOCATORをPC給電とし、磁力のない机上で見通し・同じ高さ・同じ向きに配置した。0.5 / 1.0 / 2.0 / 3.0 mを各60秒×3試行、各試行の先頭10 proceduresを除外して評価した。5.0 mは場所の時間制限により未実施とした。

| 実距離 | PBR有効 | 基本PBR中央値 | 標準偏差 | 円周位相探索中央値 |
|---:|---:|---:|---:|---:|
| 0.5 m | 153 | 0.383 m | 0.072 m | 0.380 m |
| 1.0 m | 152 | 7.991 m | 0.967 m | 7.990 m |
| 2.0 m | 150 | 6.441 m | 2.141 m | 6.475 m |
| 3.0 m | 154 | 6.662 m | 0.845 m | 6.675 m |

採用12試行では609/612 procedures（99.5%）が有効で、firmware errorは0件だった。一方、2.0 mの初回2試行と3.0 mの1試行では、checksum不一致、record sequence gap、行の結合・欠落を伴うUART破損が発生したため、ログを保存したまま距離評価から除外した。UART完全性はRF距離評価から分離して追加検証する。

基本PBR中央値と真距離のSpearman順位相関は0.40で、基準0.90を満たさなかった。RTT中央値も3.897 / 11.837 / 11.750 / 10.134 mで単調ではない。NCS v3.2.3の基本実装との照合では、IQ複素積、周波数sort、phase unwrap、線形回帰、距離換算式に差はなかった。逐次unwrapを使わない0～10 mの円周位相探索、外れchannel除去、0.5 mを基準にしたchannel別位相補正も距離順位を改善しなかった。

よって次のように判定する。

- CS通信、raw観測、採用試行の解析: 合格
- 0.5～3.0 mの距離順序: 不合格
- 絶対距離精度: 不合格
- 固定offset・単純channel補正の一般化: 不成立
- ゾーン判定への移行: 保留

正式な条件、除外データ、比較結果、図は次に保存する。
...
2026-09-16時点では、CS通信・raw観測経路は成立しているが、6.6節の見通し0.5～3.0 m距離順序が不合格である。本節に列挙したPhase 0全体の完了条件は満たしていないため、Phase 0全体を完了扱いにしない。
