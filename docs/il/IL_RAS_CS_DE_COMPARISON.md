...
以上から、真値1 m付近の弱い肩をピークとみなす方式、早期ピークの閾値調整、位相外れ値係数の調整は候補から外す。次の検証仮説は、支配IFFTとロバスト位相が共有する約3.5 mの実効遅延を機器・配置校正値として扱い、両方式の一致度を品質判定に使用する方法とする。ただし1 m一点だけで補正式は採用しない。同じ向き・高さ・見通しを維持した0.5 m / 2.0 m各1回の75-tone rawで、未補正値が真距離差に追従するかを確認する。追従しない場合は、現構成の絶対距離推定を打ち切り、接近・離隔など相対変化用途へ限定する。
