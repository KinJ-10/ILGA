...
DKをいったん移動し、同じ設置条件を再現して0.5 / 1.0 / 1.5 mを再取得した。完全フレームは各21件で、取得中のBLE / CS切断はなかった。

| 真距離 | 公式IFFT 初回 / 再試験 | 初回との差 | 公式位相 初回 / 再試験 | 初回との差 |
|---:|---:|---:|---:|---:|
| 0.5 m | 1.464 / 1.814 m | +0.350 m | 3.301 / 3.325 m | +0.024 m |
| 1.0 m | 3.220 / 2.165 m | -1.055 m | 4.699 / 4.665 m | -0.034 m |
| 1.5 m | 5.114 / 2.928 m | -2.186 m | 6.525 / 5.947 m | -0.578 m |

再試験の3距離はいずれの方式でも距離順序を維持した。ただし公式IFFTは再設置でgainが大きく変わり、初回の補正式をそのまま使える再現性はなかった。再試験の0.5 mと1.5 mだけで一次補正し、未使用の1.0 mを予測すると、公式IFFTは0.815 m（誤差-0.185 m）、公式位相傾きは1.011 m（誤差+0.011 m）だった。

公式位相傾きは0.5 mと1.0 mで初回との差が4 cm未満で、再試験内の中央距離予測も良好だった。1.5 mでは0.578 mの生値差があり、配置・反射環境への依存は残る。したがって候補順位を変更し、公式位相傾きの時間窓中央値を第一候補、公式IFFTを補助的な経路安定性指標とする。現段階は「0.5～1.5 mの距離順序と再試験内校正可能性を確認、固定の共通校正式は未成立」と判定する。
