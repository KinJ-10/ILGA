"""
BLE BMI270 Realtime Viewer (PC side)
...
Auto-detect rule (from discovered characteristic UUID suffix):
  ACC: ...ef11 or ...de11
  GYR: ...ef12 or ...de12
...
DEVICE_NAME_SUBSTRING = "BMI270_BLE_SAMPLE"   # e.g. "BMI270_BLE_SAMPLE" or "TAG"
...
# ACC display (g)
ACC_Y_LIM_G = (-2.2, 2.2)  # for ±2g full-scale (leave a margin)

# GYR display (dps)
# Stationary drift is usually within a few dps to tens of dps.
# If you want full-scale view for ±500dps, set (-550, 550).
...
class Ring:
    """Simple ring buffer for fixed-length time series."""
    ...
    def get_window(self, now: float, window_sec: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self._filled:
            idx = np.concatenate(
                [np.arange(self._i, self.length), np.arange(0, self._i)])
        else:
        ...
        t = t[mask] - now  # relative time: [-window, 0]
...
def _uuid_suffix(u: str) -> str:
    ...
    return u.split("-")[-1]  # last 12 hex digits for 128-bit UUID


def _pick_acc_gyr_chars(chars: List) -> Tuple[Optional[str], Optional[str]]:
    """
    Choose ACC/GYR characteristic UUIDs from discovered characteristics.
    Match by UUID suffix:
      ACC: ...ef11 or ...de11
      GYR: ...ef12 or ...de12
    """
    acc_uuid = None
...
async def ble_worker(sample_q: Queue, stop_evt: threading.Event) -> None:
    device = None
    print(
        f"[BLE] Scanning for '{DEVICE_NAME_SUBSTRING}' for {SCAN_TIMEOUT_SEC:.1f}s ...")
    ...
    if device is None:
        print("[BLE] Device not found. Try increasing SCAN_TIMEOUT_SEC or change DEVICE_NAME_SUBSTRING.")
        return
    ...
    async with BleakClient(device) as client:
        ...
        def on_acc(_: int, data: bytearray):
            ...
            sample_q.put(AccSample(t=now, seq=seq,
        ...
        def on_gyr(_: int, data: bytearray):
            ...
            sample_q.put(GyrSample(t=now, seq=seq,
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                await client.stop_notify(gyr_uuid)
            except Exception:
                pass
            print("[BLE] Notifications stopped. Disconnecting...")
...
def main() -> None:
    ...
    last_acc = (0.0, 0.0, 0.0)  # g
    last_gyr = (0.0, 0.0, 0.0)  # dps
    ...
    def update(_frame):
        ...
        acc_text.set_text(
            f"g   : Ax={ax_g:+.3f}  Ay={ay_g:+.3f}  Az={az_g:+.3f}")
        acc_text2.set_text(
            f"m/s²: Ax={ax_g*G0:+.3f}  Ay={ay_g*G0:+.3f}  Az={az_g*G0:+.3f}")
    ...
    _ = FuncAnimation(fig, update, interval=30, blit=False)
