"""
BLE BMI270 Realtime Viewer (PC side) - v2 (Bleak API compatibility)

Fix:
- Some Bleak versions don't have BleakClient.get_services(). This script supports:
    - await client.get_services() (if available)
    - client.services property (if available)
2
What it does:
...
async def ble_worker(sample_q: Queue, stop_evt: threading.Event) -> None:
    print(f"[BLE] bleak version: {bleak.__version__}")
