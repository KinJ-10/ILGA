...
def main():
    ...
    # --- 右下に「Save CSV」ボタンを追加 ---
    #     ax_gyr_val の右側あたりに小さいボタンを配置するイメージ
    # [left, bottom, width, height]（図全体に対する比率）
    button_ax = fig.add_axes([0.80, 0.03, 0.15, 0.05])
    save_button = Button(button_ax, "Save CSV")
    ...
    def on_save(event):
        ...
        # 開始時刻からのタイムスタンプを使ったファイル名
        t_str = time.strftime("%Y%m%d_%H%M%S", time.localtime(t0))
        filename = os.path.join(LOG_DIR, f"bmi270_log_{t_str}.csv")

        # CSV に書き出し
        with open(filename, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "time_sec",      # 計測開始からの経過時間 [s]
                "seq",
                "ax_ms2", "ay_ms2", "az_ms2",
                "ax_g", "ay_g", "az_g",
                "gx_dps", "gy_dps", "gz_dps",
    ...
    last_gx_dps = last_gy_dps = last_gy_dps = last_gz_dps = 0.0
